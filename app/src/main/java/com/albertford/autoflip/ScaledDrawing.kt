...
fun drawHorizontal(canvas: Canvas, y: Float, startX: Float, endX: Float, paint: Paint) {
    canvas.drawRect(startX * canvas.width, y * canvas.height - 0.5f, endX * canvas.width,
            y * canvas.height + 0.5f, paint)
}

fun drawVertical(canvas: Canvas, x: Float, startY: Float, endY: Float, paint: Paint) {
    canvas.drawRect(x * canvas.width - 0.5f, startY * canvas.height, x * canvas.width + 0.5f,
            endY * canvas.height, paint)
}
