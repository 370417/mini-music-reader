...
class EditPageView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    private fun onActionUp(page: Page, touch: PointF): Boolean {
        ...
        return when (motion) {
            is FirstBarDrag -> {
                ...
                if (this.motion != null) {
                    listener?.initalSelection(page.pageIndex)
                } else {
    ...
    private fun touchHandle(touch: PointF, rect: RectF): Motion? {
        ...
        return when {
            vertical != null && horizontal != null -> FirstBarDrag(
                    vertical, horizontal)
            vertical != null -> FirstBarVertical(rect,
                    vertical)
            horizontal != null -> FirstBarHorizontal(rect,
                    horizontal)
            else -> null
        }
    }
